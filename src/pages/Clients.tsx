import { Building2, Edit, Link2, Mail, Phone, Plus, Trash2, User, Users } from 'lucide-react';
...
export function Clients() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-50 dark:bg-slate-950 overflow-hidden">
      <PageHeader
        title="Clientes"
        subtitle="Gestiona tus clientes y contactos principales."
        search={{
          value: searchQuery,
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 border-r border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="p-6 text-center text-gray-500 dark:text-slate-500">No se encontraron clientes</div>
          ) : (
            <div className="divide-y divide-gray-100 dark:divide-slate-800">
              {filteredClients.map(client => (
                <div
                  ...
                  className={`p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors ${selectedClient?.id === client.id ? 'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-600 dark:border-blue-500' : ''
                    }`}
                >
            ...
            <div className="space-y-6">
              <Card className="group">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{selectedClient.name}</h2>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="cursor-pointer p-2 text-gray-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                    >
                      <Edit size={16} />
                    ...
                    <button
                      ...
                      className="cursor-pointer p-2 text-gray-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                ...
                {selectedClient.notes && (
                  <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
                    ...
                    <p className="text-sm text-gray-600 dark:text-slate-400">{selectedClient.notes}</p>
                  </div>
                )}
              </Card>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Card key={contact.id} className="hover:shadow-md transition-shadow flex justify-between items-center group">
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center">
                          <User size={20} className="text-gray-500 dark:text-slate-500" />
                        </div>
                        <div>
                          <h4 className="font-medium text-gray-900 dark:text-white">{contact.name}</h4>
                          <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">{contact.position}</p>
                          <div className="mt-2 space-y-1 text-sm text-gray-500 dark:text-slate-500">
                            {contact.email && (
                              <div className="flex items-center gap-2">
                                <Mail size={14} />
                                {contact.email}
                      ...
                      <div className="flex items-center gap-2">
                        <button className="p-2 text-gray-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                          onClick={() => {
                        ...
                        <button
                          ...
                          className="p-2 text-gray-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                  ...
                  {contacts.length === 0 && (
                    <div className="col-span-2 text-center py-8 bg-white dark:bg-slate-900/50 rounded-lg border border-dashed border-gray-300 dark:border-slate-800 text-gray-500 dark:text-slate-500">
              ...
              <div>
                <div className="flex items-center justify-between mb-4 mt-8 pt-6 border-t border-gray-100 dark:border-slate-800">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                    <div className="bg-indigo-100 dark:bg-indigo-900/30 p-1 rounded-md text-indigo-600 dark:text-indigo-400">
                      <Link2 size={18} />
                    </div>
                    Acceso al Portal
                  </h3>
                  <button
                    className='cursor-pointer bg-white dark:bg-slate-950 border border-gray-200 dark:border-slate-800 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-900 p-2 rounded-lg flex items-center gap-2 transition-colors text-sm font-medium'
                ...
                <div className="bg-blue-50/50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-900/30 rounded-lg p-4 mb-4 text-sm text-blue-800 dark:text-blue-300">
                  <p>
                ...
                <div className="space-y-3">
                  {linkedUsers.map(user => (
                    <div key={user.id} className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg hover:border-indigo-200 dark:hover:border-indigo-500 transition-colors group">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-indigo-50 dark:bg-indigo-900/30 rounded-full flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-bold text-sm">
                  ...
                  {linkedUsers.length === 0 && (
                    <div className="text-center py-6 text-gray-400 dark:text-slate-500 text-sm border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-lg">
