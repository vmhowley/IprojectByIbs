...
import { Ticket, Project, Attachment } from '../types';
...
export function ProjectDetail() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log("🚀 ~ loadData ~ projectData:", projectData)
          ...
          <div className="w-80 border-l border-gray-200 bg-white overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-2">Progress</label>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <div className="w-5 h-5 bg-gray-900 rounded flex items-center justify-center text-white text-xs">
                      D
                    </div>
                    <span>{selectedTicket.category || 'API Documentation'}</span>
                  </div>
                </div>

                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-2">Task owner</label>
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{selectedTicket.assigned_to || 'Priya'}</span>
                  </div>
                </div>

                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-2">Urgency</label>
                  <UrgencyBadge urgency={selectedTicket.urgency} />
                </div>

                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-2">Department</label>
                  <div className="flex items-center gap-2 text-sm">
                    <div className="w-5 h-5 bg-gray-900 rounded"></div>
                    <span>{selectedTicket.department || 'Engineering'}</span>
                  </div>
                </div>

                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-2">Date added</label>
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{new Date(selectedTicket.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{selectedTicket.deadline ? new Date(selectedTicket.deadline).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'May 15, 2025'}</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-3">Discussion</label>
                  <div className="space-y-3">
                    <div className="flex gap-2">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-purple-400 flex-shrink-0"></div>
                      <div>
                        <div className="text-xs font-medium text-gray-900">David Lee</div>
                    ...
                    <div className="flex gap-2">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-400 to-pink-400 flex-shrink-0"></div>
                      <div>
